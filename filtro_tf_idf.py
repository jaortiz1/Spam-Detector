...
def entrenar_tf_idf(clean_subjects,number_spams, number_ham):
    ...
    y_pred_tf_idf = knc.predict(x_val)     # Vector con la clasificación del filtro
    y_true_tf_idf = []                     # Vector con los resultados que debería dar el filtro
    for i in range(2374):
        y_true_tf_idf.append(1)
    for i in range(2708):
        y_true_tf_idf.append(0)
    y_true_tf_idf = np.array(y_true_tf_idf)
    
    
    cm = confusion_matrix(y_true_tf_idf, y_pred_tf_idf)
    f, ax = plt.subplots(figsize =(6,5))
    sns.heatmap(cm,annot = True,fmt = ".0f",ax=ax,cmap="Blues")
    plt.xlabel("y_pred_tf_idf")
    plt.ylabel("y_true_tf_idf")
    plt.show()
